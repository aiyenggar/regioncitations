...
bysort patent_id group_id: keep if _n == 1
keep patent_id group_id year_grant
destring patent_id, replace
bysort year_grant group_id: gen grp_gyear = _N
bysort group_id year_grant: keep if _n == 1
drop patent_id
bysort group_id: egen gtot = sum(grp_gyear)
merge m:1 group_id using cpc_group, keep(match master) nogen
...
use patent_date_cpc.dta, clear
keep if (group_id == "H01M" /*| group_id == "H02J"*/)
...
save electricpower.dta, replace
