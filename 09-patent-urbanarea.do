...
sort urban_area2
egen index_urbanarea2 = group(urban_area2)
sort index_urbanarea2 year
bysort index_urbanarea2 year: gen dup_patent_count=_N

sort patent_id index_urbanarea2
bysort patent_id index_urbanarea2: gen patent_region_index=_n if !missing(index_urbanarea2)
bysort patent_id index_urbanarea2: gen patent_region_count=_N if !missing(index_urbanarea2)
bysort patent_id: gen inventor_count=_N
/* inventor_count counts the total number of inventors on a patent irrespective
   of whether the inventor is mapped to an urban_area2 or not */
...
/* The variable ua2_patentinventor_total represents the total number of
   inventors on the patent who are located within an(y) urban_area2. This is
   different from the number of inventors by the number of inventors who are
   outside an urban_area2 */
bysort patent_id index_urbanarea2 (ua2_patentinventor_total): replace ua2_patentinventor_total = ua2_patentinventor_total[1] if missing(ua2_patentinventor_total) 
/* Note that the above replacement will only happen for those observations where
   index_urbanarea2 (urban_area2) is set. Those not in an urban_area2 will not have this */
   
gen share_urban_area2 = ua2_patentinventor_total/inventor_count
label variable share_urban_area2 "[patent] share of inventors in urbanareas"

replace city="" if city=="NULL"
gen latlong1=string(round(latitude,0.1))+","+string(round(longitude,0.1)) if missing(index_urbanarea2) & !missing(latitude) & !missing(longitude)
egen icity = group(latlong1) if missing(index_urbanarea2) & !missing(latlong1)
/* We want the city measures to be set only when region is unavailable */
sort icity year
bysort icity year: gen city_year_count=_N if missing(index_urbanarea2)
sort patent_id icity
bysort patent_id icity: gen patent_city_index=_n if missing(index_urbanarea2) & !missing(latlong)
bysort patent_id icity: gen patent_city_count=_N if missing(index_urbanarea2) & !missing(latlong)
...
bysort index_urbanarea2 year: egen mean_invention_weight = mean(geo_strength)
bysort index_urbanarea2 year: egen mean_inventor_count = mean(geo_count)
bysort index_urbanarea2 year: egen mean_share_urban_area2 = mean(share_urban_area2)
label variable mean_share_urban_area2 "[urbanarea-year] share of inventors in urbanareas (avg)"
...
foreach icat of local catlev {
	bysort index_urbanarea2 year: gen runsumcat`icat'=sum(1)  if nber_cat==`icat'
	replace runsumcat`icat'=0 if missing(runsumcat`icat')
	bysort index_urbanarea2 year: egen cat`icat'=max(runsumcat`icat')
	drop runsumcat`icat'
}

levels nber_subcat, local(subcatlev)
foreach l of local subcatlev {
	bysort index_urbanarea2 year: gen runsumsubcat`l'=sum(1)  if nber_subcat==`l'
	replace runsumsubcat`l'=0 if missing(runsumsubcat`l')
	bysort index_urbanarea2 year: egen subcat`l'=max(runsumsubcat`l')
	drop runsumsubcat`l'
}


/* We dropp those observations missing urbanarea */
drop if missing(index_urbanarea2)
/* Without the nber step, 3,462,342 observations deleted and 6,695,613 remain */
bysort index_urbanarea2 year: gen patent_count=_N
bysort index_urbanarea2 year: keep if _n == 1
...
drop patent_id
order year urban_area2 mean_share_urban_area2 mean_invention_weight mean_inventor_count patent_count dup_patent_count dcat* dsubcat* country* urban_area distance latitude longitude

sort index_urbanarea2 year
egen rate12 = mean(patent_count) if (year > 2000 & year <= 2012), by(index_urbanarea2)
bysort index_urbanarea2: gen pool_dup_patent_count=sum(dup_patent_count)
replace pool_dup_patent_count = pool_dup_patent_count - dup_patent_count
bysort index_urbanarea2: gen pool_patent_count=sum(patent_count)
...
sort urban_area2 year pool_patent_count
order urban_area2 year patent_count pool_patent_count cat* subcat* dcat* dsubcat* mean_* urban_area distance latitude longitude
...
tsset index_urbanarea2 year, yearly
